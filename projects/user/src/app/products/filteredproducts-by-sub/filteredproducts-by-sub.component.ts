...
@Component({
  selector: 'app-filteredproducts-by-sub',
  templateUrl: './filteredproducts-by-sub.component.html',
  styleUrls: ['./filteredproducts-by-sub.component.css']
})
export class FilteredproductsBySubComponent {
  products: Product[];
  categories :Category[];
  ...
  ListProducts() {
    return this.serv.LoadProducts().subscribe(data =>
      this.products = data.filter(product=> product.subcategory==this.id)),
